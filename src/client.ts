...
if (writer) {
  ...
  const createSection = (
    heading = "새 섹션 제목",
    paragraphs = ["새 문단을 입력하세요."],
    railClass = "image-material",
    railImage = "",
    hideRailImage = false,
    railTitle = "좌측 레일 제목",
    railText = "좌측 레일 설명을 이곳에서 직접 수정합니다.",
    sectionImageClass = "",
    sectionImage = "",
    hideSectionImage = true,
    sectionImageCaption = ""
  ...
  const applyArticle = (index: number): void => {
    ...
    if (container) {
      ...
      article.sections.forEach((section) => container.append(createSection(
        ...
        section.railTitle?.ko || section.heading.ko,
        section.railText?.ko || section.paragraphs.ko[0] || "",
        section.sectionImageClass || "",
        section.sectionImage || "",
        Boolean(section.hideSectionImage),
        section.sectionImageCaption?.ko || ""
      )));
    }
