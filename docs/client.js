...
if (writer) {
    ...
    const initialIssue = () => {
        ...
        if (stored) {
            try {
                return normalizeIssue(JSON.parse(stored));
            }
            catch {
        ...
        try {
            return normalizeIssue(JSON.parse(issueData?.textContent || "{}"));
        }
        catch {
