...
const isRuntimeContentSnapshot = (value) => {
...
const readCachedRuntimeSnapshot = () => {
    try {
...
const writeCachedRuntimeSnapshot = (snapshot) => {
...
const renderRuntimeHabitusBoard = (locale) => `      <div class="habitus-board" data-reveal data-habitus-board tabindex="0" role="img" aria-label="${runtimeEscapeHtml(locale === "ko" ? "경험과 반복의 교집합에서 습관이 생기고 선택으로 이어지는 아비투스 도식" : "A habitus diagram where the overlap of experience and repetition forms habit and leads to choice")}">
        <svg class="habitus-system" viewBox="0 0 960 430" aria-hidden="true" focusable="false">
          <g class="habitus-venn-flow">
            ...
            <path class="habitus-venn-lens" d="M480 34.35 A126 126 0 0 1 480 265.65 A126 126 0 0 1 480 34.35 Z" />
...
const waitForGalleryImage = (item) => {
    const image = item.querySelector("[data-image-source]");
...
const initGalleryElement = (gallery, readyKey) => {
    ...
    let transitionId = 0;
    const setGalleryIndex = async (index) => {
    ...
    void setGalleryIndex(Number(gallery.dataset.galleryIndex || 0));
};
const runtimeInitGalleries = (root) => {
    root.querySelectorAll("[data-gallery]").forEach((gallery) => {
        initGalleryElement(gallery, "runtimeGalleryReady");
    });
};
...
document.querySelectorAll("[data-gallery]").forEach((gallery) => {
    initGalleryElement(gallery, "galleryReady");
});
